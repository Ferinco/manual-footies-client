...
console.log(userEmail, pass)
  ...
  const TryLogin = async(e)=>{
    e.preventDefault()
    try{
      const reponse = await dispatch(loginUser({
       email: userEmail,
      password: pass
      }))
      console.log(reponse)
    ...
    <div className="flex justify-between">
      <div className="md:w-1/2 w-screen h-screen">
        <div className="max-w-[320px] m-auto flex flex-col justify-center h-full gap-16">
            <h2 className="text-3xl text-center font-semibold">Lorem Ipsum Kinikan</h2>
          ...
          <p className="text-sm font-medium text-center">
                Dont't have an account? <a className=" text-blue-600" href="/sign-up">Sign up</a>
              </p>
          </form>
        </div>
      </div>
